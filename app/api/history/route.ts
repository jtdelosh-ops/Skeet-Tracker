...
export async function GET(request: Request) {
  ...
  try {
    ...
    const count = await env.DB.prepare(`SELECT COUNT(*) AS total FROM shoots WHERE ${where}`).bind(...bindings).first<{ total: number }>();
    const total = count?.total ?? 0;
    const { page, pages, offset } = historyPage(parsed.page, total);
    const result = await env.DB.prepare(`SELECT id, shoot_number AS shootNumber, name, date, status FROM shoots WHERE ${where} ORDER BY date DESC, id DESC LIMIT ? OFFSET ?`)
      .bind(...bindings, PAGE_SIZE, offset).all<typeof shoots.$inferSelect>();
